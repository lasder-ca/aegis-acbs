...
## v0.6 projection-potential lemma

Let `d` be the normalized 3D chord direction from source to target and define the real-valued projection potential:

```text
p(v) = 2 R r <q(v), d>
```

where `r` is a conservatively reduced global minimum cost per meter. For every edge `u→v`:
