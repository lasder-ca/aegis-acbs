...
GRAPH="${1:-}"
VALIDATION_ROOT="${2:-}"
OUT="${3:-$ROOT/artifacts/v0111-release-gate}"
...
GOMAXPROCS=1 "$BIN" replay-regret \
  ...
  --output "$OUT/regret-replay-v0111.json" \
  --csv "$OUT/regret-replay-v0111.csv" \
  --html "$OUT/regret-replay-v0111.html"

GOMAXPROCS=1 "$BIN" benchmark \
  --graph "$GRAPH" \
  --queries "$QUERIES" --repeats "$REPEATS" --batch 1 \
  --order interleaved \
  --algorithms aegis,aegis-connect-32,aegis-connect-40,aegis-connect-32x16 \
  --suite mixed --pair-mode strongly-connected \
  --seed "$SEED" --timeout "$TIMEOUT" \
  --output "$OUT/benchmark-v0111.json" \
  --html "$OUT/benchmark-v0111.html"

"$ROOT/scripts/check-v0111-release-gate.py" \
  "$OUT/regret-replay-v0111.json" \
  "$OUT/benchmark-v0111.json" \
  "$OUT/release-gate.json" | tee "$OUT/release-gate.txt"
