...
OLD_TAG=v0.11.0-experimental QUERIES=100 REPEATS=5 BATCH=8 scripts/compare-tags.sh artifacts/hatfield-uk.aegis artifacts/tag-comparison
...
cp artifacts/tag-comparison/old.html "$DIST/tag-comparison-v0.11.0.html"
cp artifacts/tag-comparison/current.html "$DIST/tag-comparison-v0.11.1.html"

OLD_TAG=v0.11.0-experimental BENCHTIME=20x COUNT=3 scripts/compare-allocations.sh artifacts/allocation-comparison
cp artifacts/allocation-comparison/summary.json artifacts/allocation-comparison/summary.md "$DIST/"
cp artifacts/allocation-comparison/old.txt "$DIST/allocation-v0.11.0.txt"
cp artifacts/allocation-comparison/current.txt "$DIST/allocation-v0.11.1.txt"
...
info={
  ...
  'tests':'go test ./...','vet':'go vet ./...','benchmark':'Hatfield real OSM-derived fixture with deterministic interleaved order, ACBS ablations, allocation telemetry, a three-seed distance/time matrix, a v0.11.0/v0.11.1 tag comparison, query-level regret diagnosis, an isolated v0.11.0/v0.11.1 allocation regression comparison and multi-seed regret validation with confidence bounds, and isolated outlier replay with chunk-level traces, and concurrent stress verification'
}
