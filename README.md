...
```bash
cd /mnt/c/Users/danda/Downloads/aegis-acbs-0.5.0-experimental
go test ./...
...
## v0.5 memory behavior

ACBS now keeps both priority-queue backing arrays inside the pooled bidirectional workspace and uses a specialized binary heap without interface boxing. Path reconstruction allocates one exact-sized result slice. After workspace warm-up, a normal ACBS query therefore allocates only the returned path in the search core.

Reproduce the v0.4/v0.5 allocation comparison:

```bash
scripts/compare-allocations.sh
```

The benchmark uses a generated 180×180 road grid and reports latency, bytes per operation, and allocations per operation. Real-road results remain the publication metric; the generated grid is only an allocation-regression fixture.
