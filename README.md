...
```bash
cd /mnt/c/Users/danda/Downloads/aegis-acbs-0.11.0-experimental
go test ./...
...
- `aegis-late-guard`: 上界発見が極端に遅い時間ルートだけで最大8 chunkの均衡scheduler退避を試す実験変種
...
通常経路ではtraceを記録しません。`replay-regret`だけが、方向、辺budget、下界上昇、正規化work、frontierサイズ、効率score、上界発見chunk、late guard発火状態を保存します。v0.11では`aegis-late-guard`も同時に隔離測定し、`improved` / `neutral` / `regressed`を出力します。

## v0.11 GitHub公開ゲート

既存の東京tail検証ディレクトリを使い、11件の隔離再測定、`aegis`と`aegis-late-guard`の10,000クエリ比較、公開判定を一括実行します。
...
公開条件とalpha・stableへの昇格条件は[`docs/RELEASE_PLAN.md`](docs/RELEASE_PLAN.md)に固定しています。
...
- [`docs/RELEASE_PLAN.md`](docs/RELEASE_PLAN.md): GitHub公開・pre-release・alpha・stableの判定条件
